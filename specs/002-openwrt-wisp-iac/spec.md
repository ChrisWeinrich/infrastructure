...
Each change should have a verification checklist and a recovery plan. - Start from a read-only snapshot of the current router config before
making declarative changes. Acceptance criteria: - Router remains
connected to upstream Wi-Fi via Repeater/WISP after applying the IaC. - A client connected to the MT6000 can resolve DNS and reach the
